...
# Step 1: Define pollutant groups
solid_pollutants = ['PM2.5', 'PM10']
gas_pollutants = ['SO2', 'NO2', 'CO', 'O3']

# Step 2: Calculate total contribution for each group
solid_total = df[solid_pollutants].mean().sum()
gas_total = df[gas_pollutants].mean().sum()

# Step 3: Create data for pie chart
labels = ['Solid Pollutants', 'Gas Pollutants']
values = [solid_total, gas_total]

# Step 4: Sort values to assign color correctly (green = low, red = high)
sorted_indices = sorted(range(len(values)), key=lambda i: values[i])
colors = ['green', 'red'] if values[0] < values[1] else ['red', 'green']  # Assign red to higher, green to lower

# Step 5: Explode slices for clarity
explode = [0.1, 0.1]
...
plt.title("Comparison of Solid vs Gas Pollutants\n(Red = Higher Contribution, Green = Lower)", fontsize=13)
plt.axis('equal')
plt.show()
...
# Step 1: Create datetime column if not present
df['date'] = pd.to_datetime(df[['year', 'month', 'day']])

# Step 2: Identify whether each date is a weekend or weekday
df['day_type'] = df['date'].dt.dayofweek.apply(lambda x: 'Weekend' if x >= 5 else 'Weekday')

# Step 3: Group by month and day_type, calculate mean temperature
monthly_temp = df.groupby(['month', 'day_type'])['TEMP'].mean().reset_index()

# Step 4: Plot with Seaborn
plt.figure(figsize=(12, 6))
sns.barplot(data=monthly_temp, x='month', y='TEMP', hue='day_type', palette=['red', 'green'])

plt.title("Month-wise Comparison of Average Temperature on Weekdays vs Weekends")
plt.xlabel("Month")
...
# Step 1: Mapping stations to area types
station_to_area = {
    'Wanshouxigong': 'Urban',
    'Wanliu': 'Suburban',
    'Dingling': 'Rural',
...
# Step 2: Map area type to dataframe
df['area_type'] = df['station'].map(station_to_area)

# Step 3: Group by year and area_type and calculate average temperature
area_year_temp = df.groupby(['year', 'area_type'])['TEMP'].mean().reset_index()
...
# Step 6: Enhance with titles and labels
plt.title('Year-wise Average Temperature Rise by Area Type', fontsize=16, fontweight='bold')
plt.xlabel('Year', fontsize=12)
plt.ylabel('Average Temperature (°C)', fontsize=12)
plt.legend(title='Area Type', title_fontsize='13', fontsize='11')
...
df = pd.read_csv("/content/drive/MyDrive/Assessment_Final_Data/beijing_cleaned.csv")
...
# --- Step 5: Apply PCA to reduce dimensionality (Optional, can be skipped) ---
# pca = PCA(n_components=3)  # Uncomment if you want to reduce dimensions
